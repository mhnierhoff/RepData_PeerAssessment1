...
The **mean** is `r format(meanTotal, scientific=F)` and the **median** is `r format(medianTotal, scientific=F)` for the total number of steps per day.
...
```
The 5-minute interval, on average across all days in the dataset, that contains the maximum number of steps is `r maxInterval`.
...
The *new* **mean** is `r format(meanTotalNew, scientific=F)` and the *new* **median** is `r format(medianTotalNew, scientific=F)` for the total number of steps per day.
